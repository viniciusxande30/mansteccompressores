@php
    $isHomePage = trim(app('request')->getPathInfo(), '/') === '';
@endphp

        </main>

        <div class="fx-gap-12"></div>

        <div class="fx-footer-1-area fix bg-default" data-background="{{ url('/') }}/assets/img/footer/f2-bg-1-opt.webp">
            <div class="container fx-container-1">
                <div class="fx-footer-1-widget pt-120 pb-95">
                    <div class="fx-footer-1-widget-single has-footer-2">
                        <div class="fx-footer-1-logo fix txxaslideup">
                            <a class="txxaslideup-item fx-cube-1" href="{{ url('/') }}" aria-label="Manstec Compressores">
                                <img src="{{ url('/') }}/assets/img/manstec-opt.webp" alt="Manstec Compressores de Ar" width="160" height="160" loading="lazy" decoding="async" style="width:160px;height:auto;">
                            </a>
                        </div>
                    </div>

                    <div class="fx-footer-1-widget-single has-footer-2">
                        <h3 class="widget-title fx-heading-1 has-clr-white">Nossos links:</h3>

                        <ul class="fx-footer-1-menu has-footer-2">
                            <li><a href="{{ url('/') }}">Home</a></li>
                            <li><a href="{{ url('/') }}/sobre-nos">Sobre Nós</a></li>
                            <li><a href="{{ url('/') }}/compressores-de-ar">Área de Atuação</a></li>
                            <li><a href="{{ url('/') }}/contato">Contato</a></li>
                        </ul>
                    </div>

                    <div class="fx-footer-1-widget-single has-footer-2">
                        <h3 class="widget-title fx-heading-1 has-clr-white">Nossos Serviços:</h3>

                        <ul class="fx-footer-1-menu-2 has-footer-2">
                            <li><a href="{{ url('/') }}/manutencao-preventiva">Manutenção Preventiva</a></li>
                            <li><a href="{{ url('/') }}/manutencao-corretiva">Manutenção Corretiva e Diagnósticos</a></li>
                            <li><a href="{{ url('/') }}/kits-de-servico">Kits de Serviço e Peças Originais</a></li>
                            <li><a href="{{ url('/') }}/contratos-de-servico">Contratos de Serviço Fixo</a></li>
                            <li><a href="{{ url('/') }}/auditoria-e-inspecoes">Auditorias e Inspeções</a></li>
                            <li><a href="{{ url('/') }}/monitoramento-e-upgrade">Monitoramento e Upgrades</a></li>
                        </ul>
                    </div>

                    <div class="fx-footer-1-widget-single has-footer-2">
                        <h3 class="widget-title fx-heading-1 has-clr-white">Nosso Endereço</h3>

                        <ul class="fx-footer-1-contact has-footer-2">
                            <li>
                                <i class="fa-regular fa-location-dot"></i>
                                Rua Arapiraca, 311 - Jardim Napoli Itaquaquecetuba - SP 08595-620 - Brasil
                            </li>
                            <li>
                                <a href="mailto:comercial@mansteccompressores.com.br" aria-label="Enviar e-mail para Manstec">
                                    <i class="fa-regular fa-envelope"></i>
                                    comercial@mansteccompressores.com.br
                                </a>
                            </li>
                            <li>
                                <a href="tel:+351969558556" aria-label="Ligar para emergência Manstec Portugal">
                                    <i class="fa-regular fa-phone-volume"></i>
                                    +351 969 558 556
                                </a>
                            </li>
                            <li>
                                <a href="tel:+5511959781897" aria-label="Ligar para Manstec Brasil celular">
                                    <i class="fa-light fa-phone-volume"></i>
                                    +55 (11) 95978-1897
                                </a>
                            </li>
                            <li>
                                <a href="tel:+551146478222" aria-label="Ligar para Manstec Brasil fixo">
                                    <i class="fa-light fa-phone-volume"></i>
                                    +55 (11) 4647-8222
                                </a>
                            </li>
                            <li>
                                <i class="fa-regular fa-clock"></i>
                                Seg - Sex 7:30 - 17:20
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="fx-footer-1-copyright pt-40 pb-40">
                <div class="container fx-container-1">
                    <div class="fx-footer-1-copyright-flex d-flex align-items-center">
                        <p class="fx-footer-1-copyright-text fx-para-1 has-footer-2">©2025, Feito com carinho pela <a href="https://idsolutions.tec.br/" target="_blank" rel="noopener">ID Solutions</a>. Todos os direitos reservados.</p>

                        <ul class="fx-footer-1-copyright-menu has-footer-2">
                            <li><a href="{{ url('/') }}/politicas-de-privacidade">Políticas de Privacidade</a></li>
                            <li><a href="{{ url('/') }}/termos-de-uso">Termos de Uso</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>

        <script defer src="{{ URL('/') }}/assets/js/jquery-3.7.1.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/bootstrap.bundle.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/swiper-bundle.min.js"></script>
        @if (!$isHomePage)
        <script defer src="{{ URL('/') }}/assets/js/gsap.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/ScrollTrigger.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/SplitText.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/CustomEase.min.js"></script>
        <script defer src="{{ URL('/') }}/assets/js/lenis.min.js"></script>
        @endif
        <script defer src="{{ URL('/') }}/assets/js/main.js"></script>
        @include('includes.emergency-popup-script')
    </body>
</html>
